...
SecureBuffer::SecureBuffer(std::vector<unsigned char> bytes) : data_(std::move(bytes)) {
  lock_memory();
}
...
SecureBuffer::SecureBuffer(SecureBuffer&& other) noexcept
    : data_(std::move(other.data_)), locked_(false) {
  other.locked_ = false;
  lock_memory();
}
...
VaultKeys derive_keys(SecureBuffer& passphrase, const std::vector<unsigned char>& salt) {
  ...
  int rc = argon2id_hash_raw(/*t_cost*/ 3, /*m_cost KiB*/ 65536, /*parallelism*/ 1,
                             passphrase.bytes().data(), passphrase.bytes().size(), salt.data(),
...
std::filesystem::path ChunkVault::put(const std::string& purpose, const std::string& domain_or_url,
  ...
  auto salt = random_bytes(kSaltLen);  // stored for future format agility; record key currently
                                       // derived from Argon2id master.
...
void ChunkVault::use(const std::string& purpose, const std::string& domain_or_url,
  ...
  if (file.size() < magic_len + kSaltLen + kNonceLen + kTagLen)
  ...
  std::vector<unsigned char> nonce(file.begin() + magic_len + kSaltLen,
                                   file.begin() + magic_len + kSaltLen + kNonceLen);
